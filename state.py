...
class State:
    ...
    def __init__(self, board, predecessor, move):
        '''
        initializes the attributes and objects of the class
        '''
        self.board = board
    ...
    def is_goal(self):
        '''
        finds out if the tiles are the same as the goal tiles and returns
        true if so
        '''
    ...
    def generate_successors(self):
        '''
        generates the options of where the 0 or _ is able to move
        '''
        successors = []
        for m in range(len(MOVES)):
            b = self.board.copy()
            if b.move_blank(MOVES[m]) == True:
                s = State(b, self, MOVES[m])
                successors += [s]
        return successors                
